...
1.  在当前路径下创建 `atdir`，`btdir` 两个文件夹
2. 在 `atdir` 目录下创建 aa,bb,cc 三个文件
3. 分别用三种压缩方法对 `atdir` 目录进行压缩
4. 分别解压上一步产生的压缩包文件内容至 `btdir` 目录下

```bash
mkdir atdir btdir
touch atdir/aa atdir/bb atdir/cc

# 03. 三种压缩方法（针对 atdir 目录）
# 方法1：tar + gzip 压缩（生成 .tar.gz 格式，最常用）
# -z：gzip压缩；-c：创建压缩包；-v：显示过程；-f：指定压缩包名
tar -zcvf atdir.tar.gz atdir  
# 方法2：tar + bzip2 压缩（生成 .tar.bz2 格式，压缩率更高）
# -j：bzip2压缩（其他参数同上面）
tar -jcvf atdir.tar.bz2 atdir  
# 方法3：zip 压缩（生成 .zip 格式，跨平台兼容）
# -r：递归压缩目录（必须加，否则只压缩目录名，不包含内容）
zip -r atdir.zip atdir  

# 04. 分别解压到 btdir 目录（确保 btdir 已存在）
# 解压 .tar.gz 格式
tar -zxvf atdir.tar.gz -C btdir  # -x：解压；-C：指定解压目标路径（大写C）
# 解压 .tar.bz2 格式
tar -jxvf atdir.tar.bz2 -C btdir
# 解压 .zip 格式
unzip atdir.zip -d btdir  # -d：指定解压目标路径（小写d）
```
...
1. 在当前目录下利用 vi 命令创建文件 vi_demo 并打开
2. 修改 vi_demo 文件内容为: hello linux
3. 保存文件内容并退出 vi 模式
4. 查看 vi_demo 文件内容，确认修改是否完成！
